...
export default function LiveScoringPage() {
  ...
  const loadData = async () => {
    ...
    } else if (formattedTeams.length >= 2 && sessionData.status === 'live') {
      // Only create a new game if session is live (not completed/upcoming)
  ...
  // Creates a game in LOCAL STATE only - not saved to DB until first score
  const createNewGame = (team1Id: string, team2Id: string) => {
    setCurrentGame({
      // No 'id' yet - will be set when first score happens
  ...
  const handleScore = useCallback(async (memberId: string, points: 1 | 2 | 3, teamId: string) => {
    ...
    const { data: newScore } = await supabase
      .from('game_scores')
      .insert({ game_id: gameId, group_member_id: memberId, team_id: teamId, points } as any)
      .select()
      .single()
  ...
  const handleEndGame = useCallback(async () => {
    ...
    if (waitingTeam) {
      createNewGame(winner.id, waitingTeam.id)
    }
  ...
  const handleEndSession = async () => {
    ...
    // Delete any in-progress games (they haven't finished, so we don't want them)
    await supabase.from('games').delete().eq('session_id', sessionId).eq('status', 'in_progress')
